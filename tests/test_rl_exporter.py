...
def test_get_base_metadata_skips_non_actuated_joints(device):
  ...
  assert metadata["joint_names"] == list(robot.joint_names)
  assert "joint1" in metadata["joint_names"]
  assert "joint2" in metadata["joint_names"]

  # Stiffness/damping are only defined for actuated joints, in natural joint order.
  assert len(metadata["joint_stiffness"]) == len(robot.spec.actuators)
  assert len(metadata["joint_damping"]) == len(robot.spec.actuators)
