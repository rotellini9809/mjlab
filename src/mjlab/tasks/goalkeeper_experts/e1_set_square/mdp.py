...
class SetSquareCommand(CommandTerm):
  ...
  def _reset_to_ready_stance(
    ...
    from mjlab.tasks.goalkeeper_experts.e1_set_square.config.t1_23dof.env_cfgs import (
      KEEPER_SPAWN_Z,
      READY_JOINT_POS,
      READY_ROOT_QUAT,
    )
  ...
  def _debug_vis_impl(self, visualizer) -> None:
    ...
    # Home point marker: flat cylinder centered on the reward target.
    home_xy = _home_point_world_xy(self._env, "set_square")[batch]
    home_start = root_pos.clone()
    home_start[0] = home_xy[0]
    home_start[1] = home_xy[1]
    home_start[2] = float(self.cfg.viz.home_point_z)
    home_end = home_start.clone()
    home_end[2] += float(self.cfg.viz.home_point_height)
    visualizer.add_cylinder(
      home_start.cpu().numpy(),
      home_end.cpu().numpy(),
...
def desired_point_relative_xy(
  ...
  robot: Entity = env.scene[asset_cfg.name]
  command = cast(SetSquareCommand, env.command_manager.get_term(command_name))
  home_w_xy = env.scene.env_origins[:, :2] + torch.tensor(
    [command.cfg.home_point_x, command.cfg.home_point_y],
    device=env.device,
    dtype=torch.float32,
  )
  return home_w_xy - robot.data.root_link_pos_w[:, :2]
